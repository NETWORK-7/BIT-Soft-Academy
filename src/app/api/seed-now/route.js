...
export async function GET() {
  try {
    ...
    if (courseCount === 0) {
      return NextResponse.json({ 
        success: false, 
        message: "No courses found - needs seeding",
        needsSeeding: true 
      });
    }
...
export async function POST() {
  try {
    ...
    for (const courseData of PRACTICAL_COURSES) {
    ...
    for (const course of createdCourses) {
      const lessons = PRACTICAL_LESSONS[course.title] || [];
